...
def run_benchmark():
    ...
    for num_frac in test_cases:
        ...
        for id in range(NUM_SAMPLES):
            ...
            print(f"  v_0_1={time_v0_1:8.4f}s | v_bool={time_v1_2:8.4f}s")
        ...
        if times_v0_1 and times_v1_2:
            ...
            benchmark_data["data"].append({
                "num_expressions": num_frac,
                "degree": maxDeg,
                "v_0_1_times": times_v0_1,
                "v_bool_times": times_v1_2
            })
    ...
    if results:
        v0_1_times = [r["v_0_1"] for r in results if r["v_0_1"] > 0]
        v1_2_times = [r["v_bool"] for r in results if r["v_bool"] > 0]
        
        if v0_1_times and v1_2_times:
            avg_v0_1 = mean(v0_1_times)
            avg_v1_2 = mean(v1_2_times)
            faster = "v_bool" if avg_v1_2 < avg_v0_1 else "v_0_1"
            print(f"Average v_0_1: {avg_v0_1:.4f}s")
            print(f"Average v_bool: {avg_v1_2:.4f}s")
...
def plot_results(results_data):
    ...
    ax1.plot(avg_x_positions, avg_v0_1, color='#FF6B6B', linewidth=3, 
            label='v_0_1 - average', marker='D', markersize=8, 
    ...
    ax1.plot(avg_x_positions, avg_v1_2, color='#4ECDC4', linewidth=3, 
            label='v_bool - average', marker='D', markersize=8,
...
def create_summary_table(results_data, ax=None):
    ...
    columns = ['Input size', 'V_0_1 average', 'V_bool average', 'Factor']
