...
# Está todo lo original
    ...
    for f in range(len(orig["expressions"])):
        aux = []
        aux.append(0) # Numerador
        aux.append(0) # Denominador
        cantidad_por_frac.append(aux)

    for f in final["ecuaciones"]:

        # Lado izquierdo
        for intermedia in f["lado_izquierdo"]["intermedias"]:
        ...
        for factor in f["lado_izquierdo"]["factores_originales"]:
            # if factor["origen"] == "numerador": # REVISAR
            #     cantidad_por_frac[factor["fraccion"]][0] += factor["cantidad"]
            
            if factor["origen"] == "denominador":
    ...
    for f in range(len(orig["expressions"])):
        if cantidad_por_frac[f][0] != orig["expressions"][f]["values"][0]["degree"]:
            print(f"La fracción {f} no tiene todos los elementos originales de numerador: {cantidad_por_frac[f][0]}")
            return False
        
        elif cantidad_por_frac[f][1] != orig["expressions"][f]["values"][1]["degree"]:
            print(f"La fracción {f} no tiene todos los elementos originales de denominador: {cantidad_por_frac[f][1]}")
            return False
